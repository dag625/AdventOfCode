...
namespace {
    ...
    std::vector<race> get_input(const fs::path &input_dir) {
        const auto lines = read_file_lines(input_dir / "2023" / "day_6_input.txt");
         ...
         return std::views::zip(split_no_empty(lines[0], ' ') | std::views::drop(1) | std::views::transform(&parse_int),
                 ...
                 std::views::transform(&to_race) | to<std::vector<race>>();

    }

    int64_t distance(const race& r, int64_t held) {
        if (held <= 0 || held >= r.time) {
            return 0;
        }
        return held * (r.time - held);
    }
    ...
    race kerning(const std::vector<race>& races) {
        race retval{};
        for (const auto& r : races) {
//            const auto dt = digits(r.time) * retval.time + r.time;
//            const auto dd = digits(r.dist) * retval.dist + r.dist;
            retval.time = digits(r.time) * retval.time + r.time;
            retval.dist = digits(r.dist) * retval.dist + r.dist;
        }
        return retval;
    }
    ...
    */
    ...
    aoc::registration r{2023, 6, part_1, part_2};

//    TEST_SUITE("2023_day06") {
//        TEST_CASE("2023_day06:example") {
//
//        }
//    }

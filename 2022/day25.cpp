...
namespace {
    ...
    std::string to_snafu(int64_t num) {
        static const auto ln5 = std::log(5.0);
        const auto ln_n = std::log(num);
        const auto log5_nd = ln_n / ln5;
        auto pow = std::ceil(log5_nd);
        std::vector<int64_t> digits;
        digits.reserve(static_cast<int>(pow) + 1);
        digits.push_back(0);
        while (pow >= 0) {
            const auto base = static_cast<int64_t>(std::pow(5.0, pow));
            int64_t val = num / base;
    ...
    */
    ...
    aoc::registration r{2022, 25, part_1};

//    TEST_SUITE("2022_day25") {
//        TEST_CASE("2022_day25:example") {
//
//        }
//    }

...
namespace aoc2020 {
    ...
    namespace {
        ...
        std::pair<int, int> count_diffs(const std::vector<int>& data) {
            int num1s = 0, num3s = 0;
            ...
            return { num1s + (data[0] == 1 ? 1 : 0), num3s + 1 + (data[0] == 3 ? 1 : 0) };
        }

        using count_map = std::unordered_map<std::size_t, int64_t>;

        int64_t get_or(const count_map& counts, std::size_t val) {
            auto found = counts.find(val);
            if (found != counts.end()) {
                return found->second;
            }
            else {
        ...
        int64_t count_paths(const std::vector<int>& data) {
            count_map counts;
            counts.insert_or_assign(0, 1);
            for (auto v : data) {
                auto num = get_or(counts, v - 1) + get_or(counts, v - 2) + get_or(counts, v - 3);
                counts.insert_or_assign(v, num);
            }
            return get_or(counts, data.back());
        }
